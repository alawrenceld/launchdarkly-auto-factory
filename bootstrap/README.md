...
| `github-action-template/` | Phase 1 front end #1: drop-in PR workflows — `auto-factory.yml` (anthropic/vega, bare `uses:`) and `auto-factory-cursor.yml` (cursor; checks the tool out + `npm ci`, since `@cursor/sdk` needs node_modules) |
